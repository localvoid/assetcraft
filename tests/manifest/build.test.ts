...
describe('computeIntegrity', () => {
  test('computes an SRI string for content', () => {
    const content = new TextEncoder().encode('console.log("hello");');
    expect(computeIntegrity(content, 'sha384')).toMatch(/^sha384-[A-Za-z0-9+/]+={0,2}$/);
    expect(computeIntegrity(content, 'sha256')).toMatch(/^sha256-[A-Za-z0-9+/]+={0,2}$/);
  });
});
