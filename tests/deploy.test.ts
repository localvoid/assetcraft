...
test('open combines multiple manifests in order', async () => {
  await withTempDir(async (dir) => {
    const htmlPath = join(dir, 'html-manifest.json');
    const jsPath = join(dir, 'js-manifest.json');
    writeManifest(htmlPath, [jsEntry('/index.html', 'index.html', 'hashH')]);
    writeManifest(jsPath, [
      jsEntry('/s/a.js', 'a.js', 'hash1'),
      jsEntry('/s/b.js', 'b.js', 'hash2'),
    ]);
...
test('resolve uses each manifest directory', async () => {
  await withTempDir(async (dir) => {
    ...
    deepEqual(
      ...
      [join(dir, 'html', 'index.html'), join(dir, 'js', 'a.js')],
    );
  });
});
  ...
  await withTempDir(async (dir) => {
    ...
    deepEqual(
      second
        ...
        .map((f) => f.path)
        .sort(),
      [join(dir, 'first', 'app.js'), join(dir, 'second', 'app.js')],
    );
  });
});
...
test('open passes on same url+hash, throws on reuse', async () => {
  await withTempDir(async (dir) => {
    ...
    await (await openSingle(dir, manifest)).commit();
    // Same url+hash reopens cleanly.
    await openSingle(dir, manifest);
...
test('open throws on missing manifest file', async () => {
  await withTempDir(async (dir) => {
    await Deploy.open({
      manifests: [join(dir, 'missing.json')],
      deployPath: join(dir, 'manifest.deploy.json'),
    }).then(
      () => {
        throw new Error('expected open to throw');
...
test('added manifest source uploads everything, removed source deletes under grace', async () => {
  await withTempDir(async (dir) => {
    const jsPath = join(dir, 'js', 'manifest.json');
    const cssPath = join(dir, 'css', 'manifest.json');
    const deployPath = join(dir, 'manifest.deploy.json');
    writeManifest(jsPath, [jsEntry('/s/a.js', 'a.js', 'hash1')]);
...
test('files expands identity + variant rows', async () => {
  await withTempDir(async (dir) => {
    ...
    const deploy = await openSingle(dir, [entry]);
...
test('files skips object urls unless includeExternal', async () => {
  await withTempDir(async (dir) => {
    ...
    equal((await openSingle(dir, [entry])).files().length, 0);
    const included = await openSingle(dir, [entry], { includeExternal: true });
...
test('resolve throws on foreign entries', async () => {
  await withTempDir(async (dir) => {
    const deploy = await openSingle(dir, [jsEntry('/s/a.js', 'dist/a.js', 'hash1')]);
...
test('read reads resolved file bytes', async () => {
  await withTempDir(async (dir) => {
    const manifestPath = join(dir, 'js', 'manifest.json');
    const rel = join('dist', 'a.js');
    mkdirSync(join(dir, 'js', 'dist'), { recursive: true });
    writeFileSync(join(dir, 'js', rel), 'hello');
    ...
    equal(dirname(file.path), join(dir, 'js', 'dist'));
  });
});
...
test('commit writes to an override path', async () => {
  await withTempDir(async (dir) => {
    const deploy = await openSingle(dir, [jsEntry('/s/a.js', 'dist/a.js', 'hash1')]);
    const target = join(dir, 'elsewhere', 'deploy.json');
    await deploy.commit(target);
    const reopened = await Deploy.open({
      manifests: [join(dir, 'manifest.json')],
      deployPath: target,
    });
    equal(reopened.plan().unchanged, 1);
  });
