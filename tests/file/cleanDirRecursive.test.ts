...
import { mkdir, mkdtempDisposable, readlink, symlink } from 'node:fs/promises';
...
describe('cleanDirRecursive', () => {
  ...
  test('nested keep never follows symlinks outside dir', async () => {
    await using tmpDir = await mkdtempDisposable(join(tmpdir(), 'assetcraft-test-'));
    await using tmpOutside = await mkdtempDisposable(join(tmpdir(), 'assetcraft-test-'));
    const dir = tmpDir.path;
    const outside = tmpOutside.path;
